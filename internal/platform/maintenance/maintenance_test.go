...
func testRetention(t *testing.T, db *sql.DB, driver config.DatabaseDriver) {
	...
	if result.Sessions < 1 || result.MFAChallenges < 1 || result.Invitations < 1 || result.APITokens < 1 || result.EmailVerifications < 1 || result.PasswordResets < 1 || result.Notifications < 1 ||
